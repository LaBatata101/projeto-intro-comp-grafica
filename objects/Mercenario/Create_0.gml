#region Constructor
	...
	self.initialize("Mercenario", 
	irandom_range(3,5), // HP
	irandom_range(3,4),
	irandom_range(4,5),
	irandom_range(3,4),
	irandom_range(5,6), 
	...
	limitStrOrMag = irandom_range(12,14);
	limitSkill = irandom_range(14,16);
	limitSpd = irandom_range(12,14);
	limitLuck = irandom_range(17,19);
	limitDef = irandom_range(9,12);
	...
	self.setWeapon(["Machado de cobre", "Machado", 1, 0]);
	...
	levelUp = function() {
		while (self.xp > self.level * 40) {	// Expresão que verifica se o personagem ja tem xp para aumentar o level
			...
			} else if (temp > 35 && temp < 54) {
				if (self.luck + 1 <= self.limitLuck) {
					self.luck = self.luck + 1;
				}
			} else if (temp > 53 && temp < 61) {
				if (self.def + 1 <= self.limitDef) {
					self.def = self.def + 1;
				}
